...
CLINVAR_TSV_URL = (
    "https://ftp.ncbi.nlm.nih.gov/pub/clinvar/tab_delimited/variant_summary.txt"
)
...
def create_trimmed_variant_table(
    tsv_path: Path,
    db_path: Path,
    clinical_filter: str = "Pathogenic",
    ...
    for chunk in pd.read_csv(tsv_path, sep="\t", low_memory=False, chunksize=chunksize):
        if "ClinicalSignificance" not in chunk.columns:
            continue
        sel = chunk[
            chunk["ClinicalSignificance"]
            .astype(str)
            .str.contains(clinical_filter, na=False)
        ]
        if sel.empty:
            continue
...
def normalize_trimmed_variants(db_path: Path, batch_size: int = 100_000):
    ...
    while True:
        rows = cur.execute(
            f"select {select_clause} from variant_pathogenic limit {batch_size} offset {offset}"
        ).fetchall()
        if not rows:
            break
        inserts = []
        for r in rows:
            ...
            ref = (
                rec.get("ReferenceAlleleVCF")
                if "ReferenceAlleleVCF" in rec
                and rec.get("ReferenceAlleleVCF") not in (None, "")
                else rec.get("ReferenceAllele")
            )
            alt = (
                rec.get("AlternateAlleleVCF")
                if "AlternateAlleleVCF" in rec
                and rec.get("AlternateAlleleVCF") not in (None, "")
                else rec.get("AlternateAllele")
            )
            if (
                Chromosome in (None, "")
                or pos in (None, "")
        ...
        if inserts:
            cur.executemany(
                "insert OR IGNORE into variant_vrs (vrs_id,VariationID,GeneSymbol,vrs_input) values (?,?,?,?)",
                inserts,
            )
            conn.commit()
        offset += batch_size
