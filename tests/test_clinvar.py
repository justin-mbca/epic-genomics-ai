...
# Import after inserting repo root so tests can run from CI/local paths
from epic_etl import clinvar  # noqa: E402
...
def test_clinvar_load_and_normalize(tmp_path: Path):
    ...
    clinvar.create_trimmed_variant_table(
        tsv, db, clinical_filter="Pathogenic", chunksize=10
    )
    conn = sqlite3.connect(str(db))
