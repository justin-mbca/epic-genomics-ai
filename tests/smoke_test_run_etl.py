...
def make_test_bundle(path: Path):
    bundle = {
        "resourceType": "Bundle",
        "entry": [
            {"resource": {"resourceType": "Patient", "id": "p1", "name": [{"given": ["Alice"], "family": "Doe"}], "gender": "female", "birthDate": "1980-01-01"}} ,
            {"resource": {"resourceType": "Observation", "id": "o1", "subject": {"reference": "Patient/p1"}, "code": {"coding": [{"code": "test-code"}]}, "valueQuantity": {"value": 7.5, "unit": "mg/dL"}, "effectiveDateTime": "2020-01-01"}} ,
            {"resource": {"resourceType": "Condition", "id": "c1", "subject": {"reference": "Patient/p1"}, "code": {"coding": [{"code": "cond-code"}]}, "onsetDateTime": "2019-05-01"}} ,
            {"resource": {"resourceType": "Encounter", "id": "v1", "subject": {"reference": "Patient/p1"}, "period": {"start": "2020-01-01"}, "class": {"code": "outpatient"}}},
        ],
    }
...
def run():
    ...
    meas = cur.fetchone()[0]
    print(f"persons={persons} obs={obs} cond={cond} visits={visits} cond_occ={cond_occ} meas={meas}")
